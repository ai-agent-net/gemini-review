...
| `comment_title`     | No       | `# 🤖 Gemini AI Code Review` | A unique title to prepend to the review, used to find old comments for editing                                                                        |
...
- **Выбор модели**: Поддержка различных моделей Gemini (Pro, Flash).
...
| `agent_model`       | Нет         | `flash`               | Модель Gemini (например, `gemini-1.5-pro`, `flash`, `pro`).                                                                                         |
...
| `comment_title`     | No          | `# 🤖 Gemini AI Code Review` | Уникальный заголовок для добавления к отзыву, используется для поиска старых комментариев для редактирования                                                                      |
